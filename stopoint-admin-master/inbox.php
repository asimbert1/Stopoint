...
$table->addField('Order Number', 'OrderId', false, '', '');

$table->addJoined('User Name', 'FromId', false, 'user', 'FirstName', 'id');

$table->addField('Subject', 'Subject', false, '', '');

$table->addField('Date', 'Date', true, '', '');

$table->addField('Is Read', 'IsRead', true, '', '');

// which fields to be searchable

$table->searchable = array('id','subject','date','IsRead'); 
...
//$messages_query = mysql_query("SELECT * FROM `messages` WHERE ToId = '" . $_SESSION['login_id'] . "' and IsRead='0'");
...
$(document).ready(function(){ 
	...
	$(function() {
		...
		<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<a href="<?=$_SERVER['PHP_SELF']?>?sort=Date&desc"><h2>Messages</h2></a>
				...
				<div class="content-box-content">
						...
						<table>
							...
							<tbody>
								...
								$vasfromid = "SELECT * FROM `messages` where id =".$row['id'];

								$refromid=mysql_query($vasfromid);

								$wefromid=mysql_fetch_assoc($refromid);

								

								$vasuser = "SELECT * FROM `user` where id =".$wefromid['FromId'];

								$reuser=mysql_query($vasuser);

								$weuser=mysql_fetch_assoc($reuser);

								if($weuser['UserType']=='User'){
								...
								<tr id="events_<?=$row['id']?>">
										  ...
										  <td>
												...
												<?php 
...

													{
															...
															echo html_entity_decode(stripslashes($weuser['FirstName'].' '.$weuser['LastName']));
									...
									<td>
										 ...
										 <a href="order_edit.php?action=edit&id=<?=$wefromid['OrderId']?>" title="Edit"><img src="<?=$base_url; ?>/images/icons/pencil.png" alt="Edit" /></a>
