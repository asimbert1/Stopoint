...
//$table->addField('Name2', 'UserId', false, '', '');   
$table->addJoined('Name', 'UserId', false, 'user', 'id', 'id');
...
if($_GET['key'] == 'release')
{
	$table->addField('Payment Method', 'Returnlabel', false, '', '');
	//$table->addJoined('Payment Method', 'UserId', false, 'user', 'PaymentMethod', 'id');
}
							...
							{	
								...
								if ($key != 'id'): ?>
									<td><?php
										...
										{
											$_user_id = $value;
											$vasname = "SELECT * FROM `user` WHERE id = '".$value."'";
											$rename=mysql_query($vasname);
											$wename=mysql_fetch_assoc($rename);
											echo html_entity_decode(stripslashes($wename['FirstName'].' '.$wename['LastName']));
										}
									...
									<td><?php echo html_entity_decode(stripslashes($weproduct['Description'])); ?></td><?php
										...
										else if($key== "OrderDate")
										{
											$value = date('m/d/Y h:i A', strtotime($value));
											echo $value;
										}else if($key == "Returnlabel")
										{
											$vasname = "SELECT * FROM `user` WHERE id = '".$_user_id."'";
											$rename=mysql_query($vasname);
											$wename=mysql_fetch_assoc($rename);
											
											$_payment_method_id = $wename['PaymentMethod'];
											if($_payment_method_id !== null && $_payment_method_id !== ""){
												$_payment_method_str = $_payment_method_id == 1?"PayPal":"Check";
											}else{
												$_payment_method_str = "";
											}
											
											echo html_entity_decode(stripslashes($_payment_method_str));
										}
