...
if(isset($_GET['a_id'])){
	...
	$idd='='.$_GET['a_id'];
		...
		$form->add_hidden('live_id');
$form->add_select('Venue','venue_id','venue','id','name','name','','id'. $idd);

$form->add_text('Name: ', 'name');	
$form->add_text('Capacity: ', 'capacity');	
$form->add_dropdown('Auditorium Type','type',array(array('name'=>'Seating','value'=>'Seating'),array('name'=>'Floor','value'=>'Floor'),array('name'=>'Floor and Seating','value'=>'Floor and Seating')));
...
$fields = array('capacity','image_url','name','venue_id','type');
...
if (isset($_POST['submit'])) {
	...
	if ($id) {
		...
		$spaceadd = $venuesClient->editSpace($id, $name, $maxCapacity, $reservationType);
		
		
        	$form->setPostFields($fields,'id', $id);
			header("Location: $url?saved&p=$p&sort=id&desc&a_id=".$_GET['a_id']);
	}
	else {
		$name = $_POST['name'];
		$maxCapacity = $_POST['capacity'];
		...
		$reservationType = $_POST['type'];
		 ...
		 $sql="SELECT * FROM `venue` WHERE id = $_GET[a_id]";
		

$row1=mysql_query($sql);
$row = mysql_fetch_assoc($row1);
  $row['liveid'];
 
		$spaceadd = $venuesClient->addSpace($row['liveid'], $name, $maxCapacity, $reservationType);
		echo $_POST['liveid'] = $spaceadd;
		
			$form->setPostFields($fields,'id','',true);
			header("Location: $url?added&sort=id&desc&a_id=".$_GET['a_id']);	
	}
}
...
<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<!-- Page Head -->
			<a href="<?=$_SERVER['PHP_SELF']?>"><h2>Auditorium</h2></a>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<h3><?php echo getTitle($action); ?> Auditorium</h3>
