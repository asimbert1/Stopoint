...
$table->addField('Email', 'EmailAddress', true, '', ''); 
$table->addField('User Type', 'UserType', false, '', '');
$table->addField('Status', 'Status', false, '', ''); 

// which fields to be searchable

$table->searchable = array('id','FirstName','LastName','Status'); 
...
if (isset($_GET['suspend'])) {
		...
		$db->query("UPDATE `$table_name` SET `Status` = 0 WHERE `id` = ".sanitize($_GET['suspend']));

		$action_msg = show_notification("Account has been Suspended, succesfully","success",true);
...
if (isset($_GET['activate'])) {
		...
		$db->query("UPDATE `$table_name` SET `Status` = 1, VerificationCode = '' WHERE `id` = ".sanitize($_GET['activate']));

		$action_msg = show_notification("Account has been Activated, succesfully","success",true);
		...
		<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-content">
                        ...
                        <a href="cutomers.php" class="link_button" style="font-size: 20px;">View Users</a>-->
...
if (agree){
     ...
     return false ;
										...
										if($row['Status'] == 1){
										?>
                                        <a class="confirm_link" style="color:red;" id="suspend_link" href="<?=$_SERVER['PHP_SELF']?>?suspend=<?php echo urlencode($table->current_id);?>" onclick="return confirm('Are you sure?')" title="Suspend"><img height="16" width="16" src="<?php echo $base_url; ?>/images/icons/suspend.jpg" alt="Suspend" /></a>
                                        <?php
										}else{
										?>
                                        <a class="confirm_link" style="color:green; font-weight:bold;" id="active_link" href="<?=$_SERVER['PHP_SELF']?>?activate=<?php echo urlencode($table->current_id);?>" onclick="return confirm('Are you sure?')" title="Activate"><img height="16" width="50" src="<?php echo $base_url; ?>/images/icons/activate.jpg" alt="Activate" /></a>
                                        <?php
										}
										?>
