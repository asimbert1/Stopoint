...
if($_GET['key'] == 'adjusted' || $_GET['key'] == 'release' || $_GET['key'] == 'all' || $_GET['key'] == 'paid' || $_GET['key'] == 'returned' )
{
									...
									{ ?><?php 
										if ($key != 'id'): ?>
											...
											{ ?><?php
												...
												if($value == 10){ echo html_entity_decode(stripslashes('Activation Locked'));}?><?php
											}
											else if($key == "ProductId")
											{ ?><?php
												$vasproduct = "SELECT * FROM `product` WHERE ProductCode = ".$value;
