...
$query =  "SELECT * from user WHERE EmailAddress = '".base64_decode($id)."'";
	...
	$subject = 'Stopoint ';
	$message_body = '<p>Thank you for registering with Stopoint. You can activate your account. ';
	$to = $email;
...
<html xmlns="http://www.w3.org/1999/xhtml"><head><meta name="viewport" content="width=device-width" /></head><body>';
	...
	$message .= '<br /><a href="https://www.stopoint.com/login.php?err_msg=activate_success&status=0&id='.$id.'" target="_blank">Click Here to Activate</a>';
	...
	$message .= '</body></html>';
		...
		if($sent){
				header('Location: '.$base_url.'/login.php?err_msg=login_resend&reid='.$id);
			...
			if($weresultSet['EmailAddress'] == $email && $weresultSet['Status'] == 0){
				...
				if($checkout == "true"){
					...
					header('Location: '.$base_url.'/login.php?checkout=true&err_msg=login_resend&reid='.$id);	
				...
				if($checkout == "true"){
					...
					header('Location: '.$base_url.'/login.php?checkout=true&err_msg=login_error');	

				}

				else{

					header('Location: '.$base_url.'/login.php?err_msg=login_error');
