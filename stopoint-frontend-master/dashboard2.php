...
//**********Handling customer response for Adjusted price inspection activity (YES/NO)**********
if (isset($_GET['adjusted-price']) && isset($_GET['orderno'])) {
	$pqueryrelease = "SELECT * FROM `order` WHERE id = '" . $_GET['orderno'] . "'";
    $rpqueryrelease = mysql_query($pqueryrelease);
    $wpqueryrelease = mysql_fetch_assoc($rpqueryrelease);
	if($_GET['adjusted-price'] == "yes"){
		if($wpqueryrelease['OrderStatus'] == 6){
		
		}
	else{
    $queryaccept = mysql_query("UPDATE `order` SET `OrderStatus` = 5 WHERE id = " . $_GET['orderno']) or die(mysql_error());	//release payment
	}
	}
	if($_GET['adjusted-price'] == "no"){
		if($wpqueryrelease['OrderStatus'] == 6){
...
mysql_query("SET SQL_BIG_SELECTS=1");
$rowsorderadjustedinspection_query = mysql_query("SELECT order.id as OrderId, order.TrackingCode as TrackingCode,order.FedexCode as FedexCode, order.OrderDate as OrderDate, order.OrderAmount as OrderAmount, order.AdjustedAmount as AdjustedAmount, order.FedexCode as FedexCode, product.ProductModel as ProductModel, product.ProductCode as ProductCode, product.Description as Description, productfamily.Name as ProductName,
                                                    CASE order.OrderStatus
													WHEN '15' THEN 'Adjusted Price Inspection'
													WHEN '14' THEN 'Blacklisted'
													WHEN '13' THEN 'Inspection'
													WHEN '12' THEN 'Inspection'
													WHEN '11' THEN 'Installment Payment'
												   ...
												   WHEN '8' THEN 'Expired' 
                                                    ...
                                                    ELSE 'Pending'
                                                    END as OrderStatus
                                                   FROM `order` INNER JOIN `product` ON product.ProductCode=order.ProductId INNER JOIN `productfamily` ON product.FamilyId=productfamily.Id INNER JOIN `user` ON user.id=order.UserId INNER JOIN `productbrand` ON productbrand.id=product.BrandId INNER JOIN `productcategory` ON productcategory.id=product.CategoryId WHERE order.UserId=" . $_SESSION['login_id'] . " and order.OrderStatus=15 ORDER BY order.OrderDate desc");
$numrow_adjustedinspection = mysql_num_rows($rowsorderadjustedinspection_query);								
                        ...
                        if ($_GET['tab'] == "tab_overview") {
                         ...
                         <li class="active"><a href="#tab_overview" data-toggle="tab"><i class="fa fa-dashboard"></i>Overview</a></li>
                            ...
                            <li><a href="#tab4" data-toggle="tab"><i class="fa fa-comments"></i>Messages <?php if($numrows_message>0){ ?> <span style="color: red;">(<?=$numrows_message?>)</span> <?php }else{ ?> (<?=$numrows_message?>) <?php } ?></a></li>
							 ...
							 <li><a href="#tab10" data-toggle="tab"><i class="fa fa-key"></i>Adjusted Price Inspection <?php if($numrow_adjustedinspection>0){ ?> <span style="color: red;">(<?=$numrow_adjustedinspection?>)</span> <?php }else{ ?> (<?=$numrow_adjustedinspection?>) <?php } ?></a></a></li>
...
// loop over the rows, outputting them
while ($roworderadjust = mysql_fetch_assoc($rowsorderadjust)) {
    if($roworderadjust['OrderStatus']=='Adjusted Price Inspection')
    {
        ...
        <table class="table table-striped">
                                                ...
                                                <br />
                                                <p>SELECT ONE OF THE FOLLOWING OPTIONS: </p>
                                                    <p>1. If you paid your installment payment , please &OpenCurlyDoubleQuote;<a href="dashboard?adjusted-price=yes&orderno=<?php echo $roworderadjust['OrderId']; ?>">CLICK HERE</a>&CloseCurlyDoubleQuote; .</p> <br />																										
													
<p>2. If you are unable to pay your installment payment. Please &OpenCurlyDoubleQuote;<a href="dashboard?adjusted-price=no&orderno=<?php echo $roworderadjust['OrderId']; ?>">CLICK HERE</a>&CloseCurlyDoubleQuote; .</p><br />
 ...
 ?>
                                                    ...
                                                    <div class="row no-gutter">
                                                        ...
                                                        <div class="alert alert-danger alert-dismissible fade in" role="alert" id="review-alert">
                                                             <p> <span style="color:red;font-weight: bold"> You have not provided a feedback for your previous order. Please click on "Reviews" tab to leave a review. Thank you.</span></p>
